def PolyMAX(FRF, Freq, min_freq, max_freq, N):
    ...
    for frfs in FRF:
        ...
        for i in range(Nf):
            ...
            X_sensor.append(X_)
        #Repeat for number of input and comput the Y columns
        for 
        Y_sensor = np.kron(-X_,frfs)
        ...
        M =+ M_sensor
    M_sensor = 2* M_sensor
    alpha = -1*np.linalg.inv(M_sensor[0:N*m,0:N*m]) @ M_sensor[0:N*m, N*m:len(M_sensor)]
    Im = np.identity(m)
    alpha = np.concatenate((alpha, Im), axis = 0)
    #creatin the companion matrix
    CM = np.eye(len(M_sensor),k = 1)
