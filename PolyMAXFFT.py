def PolyMAXFFT(FRF, Freq, Coh, min_freq, max_freq, Nmin, Nmax):
    ...
    T = np.fft.fftn(np.abs(FRF*Coh)**2, s =(2*Nf,2*Nf), axes=(1, 2))
    #Computing Y
    #Y = np.fft.fftn(FRF, s =(2*Nf,2*Nf), axes=(1, 2))
    #compute M for all output
    for ls in range(l):
        X = np.fft.fftn(abs(Coh[0,:,ls]), s =(2*Nf,), axes=(0,))
        T_sensor = toeplitz(np.real(T[:,0:Nmax+1,ls]))
        S = np.fft.fftn((abs(Coh)**2)*FRF,s =(2*Nf,2*Nf), axes=(1, 2))
        S_sensor = toeplitz(np.real(S[:,0:Nmax+1,ls]))
